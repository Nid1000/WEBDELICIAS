...
@section('content')
    <section class="grid gap-6 xl:grid-cols-[1fr_320px]">
        <article class="admin-card">
            <form action="{{ route('web.admin.categories.update', $category->id) }}" method="POST" class="space-y-4">
                @csrf
                @method('PATCH')
                <div>
                ...
                <div>
                    ...
                    <input id="imagen_url" name="imagen_url" value="{{ old('imagen_url', $category->imagen) }}" class="input">
                </div>
